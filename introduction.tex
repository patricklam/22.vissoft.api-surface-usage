...
We now discuss a possible VizAPI usage scenario, from the perspective of a client developer. First, consider a client $C$ and a library $L$, in the context of plain Java. Library $L$ has packages $L_1$, $L_2$, and $L_3$. $C$ calls into $L_1$ and $L_2$. Internally, within $L$, $L_1$ and $L_2$ call into each other, but not into $L_3$. The VizAPI result, with no edges from $C$ directly to $L_3$, allows a developer to conclude that breaking changes in $L_3$ will not affect $C$. Also, if only $L_3$ uses an external dependency $D$, then we know that $C$ will not need $D$ to be on its classpath.
