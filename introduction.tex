...
On the other hand, we would expect (and have verified) that each client uses only a small portion of each of its dependencies' API surfaces. Bringing the discussion back to the breaking changes issue, we hypothesize that, most of the time, most breaking changes will not affect most clients; and that it is useful for clients to know whether they are using the parts of the API surface that are subject to a particular breaking change.
